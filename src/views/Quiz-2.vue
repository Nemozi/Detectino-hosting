...
onMounted(async () => {
  ...
  // 2. REAL IMAGES (4 Stück, ungesehen)
  ...
  // 3. STANDARD AI (3 Stück, ungesehen)
  ...
  // 4. NANOBANANA AI (3 Stück, ungesehen)
...
const handleSuccess = () => {
  ...
  else { gameFinished.value = true; finishLevel(); }
}
...
<template>
  <div class="content-wrapper">
    <div v-if="loading" class="loading-screen">
        ...
        <p>{{ t('level7.loading') }}</p>
    </div>

    <div v-else class="level-container">
      <analysis 
        v-if="!gameStarted && !gameFinished" 
        :title="t('level7.introTitle')" 
        :text="t('level7.introText')" 
        :buttonText="t('level7.startBtn')" 
        @next="gameStarted = true" 
      />

      <div v-if="gameStarted && !gameFinished && quizImages[currentRound]">
        <div class="level-progress-bar">
            <span>{{ t('level7.roundLabel') }} {{ currentRound + 1 }} / {{ roundsTotal }}</span>
        ...
        <RealOrFake
          ...
          :levelId="7"
          :questionText="t('level7.questionText')"
      ...
      <div v-if="gameFinished" class="neo-card result-card" style="text-align:center;">
        <h2 class="neo-title">{{ t('level7.endTitle') }}</h2>
        <div class="score-display">{{ score }} / 10</div>
        <p>{{ t('level7.endText') }}</p>
